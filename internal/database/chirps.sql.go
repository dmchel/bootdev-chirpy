...
func (q *Queries) DeleteChirp(ctx context.Context, id string) (Chirp, error) {
...
const getChrirpsWithAuthorId = `-- name: GetChrirpsWithAuthorId :many
SELECT id, created_at, updated_at, body, user_id
FROM chirps
WHERE user_id = $1
ORDER BY created_at
...
func (q *Queries) GetChrirpsWithAuthorId(ctx context.Context, userID string) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getChrirpsWithAuthorId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
